...
int validacao (int limite); //Prototipo declara somente o cabeçalho, em C não importa o nome do parametro somente o tipo, em outras ling. importa

int main()
{
	setlocale(LC_ALL,"portuguese");
    const int qtde = 3;   //Const não pode-se alterar o valor.
    int estadoCivil, escolaridade, solteiroSuperior, opcao, nPessoas;
    solteiroSuperior = 0;
    cout<<"\nEntre com os dados das "<<qtde<<" pessoas, conforme as opções listadas:\n";
    for(nPessoas=1; nPessoas<=qtde; nPessoas++) //nPessoas: Variavel local do for
    {
        ...
        cout<<"(5) Pós Graduado\n";
        cout<<"   Opção [ ]\b\b";
       	/*opcao=sqrt(validacao(15)); // ou opcao=validacao(sqrt(15))*/ // Valido somente para parametro de entrada 
       	opcao = validacao(5);
...
int validacao (int limite)			//int limite = parametro de entrada, consistuido de nome e tipo
{
